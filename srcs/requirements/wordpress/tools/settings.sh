...
if [ ! -c /var/www/html/index.php ]; then
	...
	sed -i '/#listen = \/var\/run\/php5-fpm.sock/a\listen = wordpress:9000' ${WWW_CONF_PATH}${WWW_CONF_FILE}
	
	wp core install --allow-root --path=${WP_PATH} --url=${DOMAIN_NAME} --title=${WORDPRESS_TITLE} --admin_user=${WORDPRESS_DB_USER} --admin_password=${WORDPRESS_DB_PASSWORD} --admin_email=${WORDPRESS_ADMIN_EMAIL}
fi

wp cli update
wp --allow-root --path=/var/www/html/wordpress rewrite structure '/%postname%/'
wp --allow-root --path=/var/www/html/wordpress rewrite flush
